...
void sendChannelDigits(int channel) {
    ...
    for (char const &c : s) {
        ...
        if (digit >= 0 && digit <= 9) {
            ...
            // wait for the TV between digits
            delay(400); 
        }
    }
}
...
String getMemoryJSON() {
    ...
    for (int i = 0; i < 14; i++) {
        if (memory[i].protocol == 0) {
            ...
            // This formats the command as a clean Hex string
            char hexBuffer[12];
...
void processCommand(String cmd) {
    if (cmd == "power")       queueIR(memory[10]);
    else if (cmd == "source")  queueIR(memory[11]);
    else if (cmd == "volup")   queueIR(memory[12]);
    else if (cmd == "voldown") queueIR(memory[13]);
}
...
void irCoreTask(void *pvParameters) {
    ...
    for (;;) {
        if (IrReceiver.decode()) {
            // Check if protocol is valid and not 0
            bool isValid = (IrReceiver.decodedIRData.protocol != UNKNOWN) && 
                           (IrReceiver.decodedIRData.decodedRawData != 0);
            
            // Check if it is a "Repeat" code (holding button down)
            bool isRepeat = (IrReceiver.decodedIRData.flags & IRDATA_FLAGS_IS_REPEAT);

            // Only process if valid and NOT a repeat
            if (isValid && !isRepeat) {
                
                if (learningIndex != -1) {
                    // We are in learning mode Save it
                    Serial.printf("Learned: 0x%08X\n", IrReceiver.decodedIRData.decodedRawData);
                    saveCode(learningIndex, &IrReceiver.decodedIRData);
                    learningIndex = -1; // Turn off learning mode
                } else {
                    // Just display it on the web dashboard
        ...
        // Handle sending from the queue
        if (xQueueReceive(irQueue, &txData, 0) == pdTRUE) {
            IrReceiver.stop();
            IrSender.write(txData, 0);
            delete txData; 
            IrReceiver.start();
        }
